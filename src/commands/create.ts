...
const options = (account?: string): IAsksTypes[] => {
    ...
    if (account) {
        return asks.map<IAsksTypes>((ask, index) => {
            if (index > 0) {
                return {
                    ...ask,
...
const command: GluegunCommand = {
    ...
    run: async toolbox => {
        ...
        prompt.ask<ICreateReturn>(asks).then(async ({ name, app, token }) => {
            ...
            const { data: { accounts } } = await api.get<InfoResponse>(`/v1/user?id=${id}`, { id });
            ...
            if (exists.length) {
                spinner.stop()
                print.newline()
                return print.warning(`${warning} ${account.name} account has already been created!`)
            }
            ...
            api.post<ICreateAccountResponse>('/v1/add', {
                ...
                if (data.error) {
                    if (data.error === "Token inválido") {
                        print.newline();
                        return print.warning(`${warning}: You need to be logged in before creating a new account, try the command: masterdata login [user]`)
                    }

                    print.newline();
                    return print.error(`${error} We were unable to execute the create command at this time, please try again.`);
                } else {
                    print.newline();
                    print.divider();
                    print.success(`${print.colors.bold.blue(account.name)} account was created successfully!`);
                    print.divider();
                }
