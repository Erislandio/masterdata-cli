...
async function switchAccount(account: string): Promise<InfoResponse> {
    ...
    return api.post<InfoResponse | any>('/v1/set/account', {
        name: account,
        id
    }).then(({ data }) => {
        return data
...
const command: GluegunCommand = {
    ...
    run: async toolbox => {
        ...
        if (array.length) {
            ...
            spinner.start(`${info} Switching to account ${array[0].toString().trim()}`);
            ...
            print.newline();
            print.divider()
            print.success(`Now you are using the ${print.colors.bold.magenta(array[0].toString().toUpperCase())} account`);
            print.newline();
            return
