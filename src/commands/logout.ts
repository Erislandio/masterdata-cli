  ...
  run: async toolbox => {
    ...
    if (!id) {
      ...
      return print.warning(
        `${warning} You need to be logged in to access this resource`
      )
    }
    ...
    prompt
      ...
      .then(res => {
        if (res) {
          ...
          return toolbox.patching
            .update(__dirname + '/../auth/variables.json', config => {
              config.token = null
              config.id = null
              return config
            })
            ...
            .catch(() => {
              print.newline()
              print.error(
                `${error} The resource could not be accessed at this time`
              )
            })
        }
